...
- **Residual, honest:** extraction still sometimes drops the user's **name** (strengthened the prompt with an explicit example, **pending live re-verify** — LM Studio crashed at commit time); an occasional "just a chatbot" slip (tightened, also pending); heavy repeated pressure can still yield a short non-repeated poem. Re-run both eval scripts once LM Studio is restarted to confirm.
